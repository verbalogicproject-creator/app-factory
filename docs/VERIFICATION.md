...
| V2 | *local typecheck* | — | **not built** — see below |
| V3 | CI compile | 2–4 min | does it compile? |
| V4 | unit tests | +30s | is the logic right? |
| V5 | R8 / minify | +90s | does minification break it? |
...
**V2, local typechecking, does not exist.** `aapt2` is the only genuinely native
blocker; Kotlin, KSP, R8 and Compose are all JVM. If it worked it would move the
missing-import class of error from a 3-minute round trip to seconds. Two such errors
have already been paid for at full price.
