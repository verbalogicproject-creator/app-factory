...
> **Status at v1.0.0 (2026-09-13).** Further correction, recorded the same way: the rows
> above are left as written, this block records what has since changed.
>
> - Row 2 (`targetSdk 34`) — **fixed.** `plugins/appfactory-core/skills/bootstrap/SKILL.md:34`
>   now recommends `36`.
> - Row 6 (marketplace advertises four plugins) — **fixed, by deletion rather than
>   implementation.** `appfactory-plan`, `appfactory-ui`, `appfactory-build` were
>   manifest-only stubs and are removed from the tree entirely. The decided shape going
>   forward is one plugin, `appfactory-core`, with four skills (`bootstrap` present;
>   `plan`, `verify`, `release` planned in v1.0.0, not yet present) — see
>   `docs/PLUGIN-ROADMAP.md`.
> - Row 7 (no `LICENSE`) — **fixed.** `LICENSE` and `NOTICE` exist at the repo root.
> - Row 8 ("no Android SDK" headline) — **fixed.** `README.md:4` now reads "with or
>   without a local Android SDK".
> - §4's "stale" rows (`targetSdk`, local-SDK framing, preflight check count) — all three
>   are now current: `targetSdk` recommendation matches Localmind's `36`; the headline no
>   longer claims no local SDK; the check count is **20** on disk (`010`–`200`), and
>   `docs/CHECKS.md`, `README.md` and `docs/GETTING-STARTED.md` all say so.
> - The term "stage ledger" used elsewhere in this repository's manifests (fixed in this
>   pass) never referred to a built artifact; the actual planned mechanism is per-stage
>   JSON receipts under `.appfactory/receipts/`, not yet present.
