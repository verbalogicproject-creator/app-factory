...
device with no Android SDK it cannot be generated locally either.

The way out is suppress-with-reason, push, let CI generate and upload the schema, commit
it, delete the suppression. Which is a legitimate use — and precisely why the reason
field is required, so the temporary suppression cannot quietly become permanent.
